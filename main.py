...
async def registration_username(message: types.Message):
    ...
    # Проверка наличия матерных слов в логине
    if check_word(registration_username) == "Слово матерное":
        await message.answer('Логин содержит матерные слова. Пожалуйста, используйте другой логин.')
        return
...
async def registration_password(message: types.Message, state: FSMContext):
    ...
    # Проверка наличия матерных слов в пароле
    if check_word(registration_password) == "Слово матерное":
        await message.answer('Пароль содержит матерные слова. Пожалуйста, используйте другой пароль.')
        return
    await state.finish()
